...
void Kinect2Node::publish()
{
  ...
  if(data) {
    ...
    rgb_registered.data = cv::Mat(depth_rectified.data.rows, depth_rectified.data.cols,
                                  CV_8UC4,
                                  cv::Scalar());
    const float rgb_fx = rgb_optimal_camera_matrix_.at<float>(0,0);
    const float rgb_fy = rgb_optimal_camera_matrix_.at<float>(1,1);
    const float rgb_cx = rgb_optimal_camera_matrix_.at<float>(0,2);
    const float rgb_cy = rgb_optimal_camera_matrix_.at<float>(1,2);
    ...
    for(int i = 0 ; i < depth_rectified.data.rows ; ++i) {
      for(int j = 0 ; j < depth_rectified.data.cols ; ++j) {
        const float depth = depth_rectified.data.at<float>(i,j);
        if(std::isnormal(depth)) {
          cv::Vec4f p (depth_lookup_rectified_x_.at<float>(i,j) * depth,
                       depth_lookup_rectified_y_.at<float>(i,j) * depth,
                       depth);
          cv::Mat p_ = kcalibration_.extr_transformation * cv::Mat(p);

          /// exchange through eigen

          double depth_ = 1.0 / p_.at<float>(2);
          const int rgb_x = (p_.at<float>(0) * rgb_fx) * depth_ + rgb_cx;
          const int rgb_y = (p_.at<float>(1) * rgb_fy) * depth_ + rgb_cy;
...
void Kinect2Node::updateCameraInfo()
{
  if(!kinterface_camera_parameters_) {
    ...
    cv::Vec2d *depth_lookup_rectified_x_ptr = depth_lookup_rectified_x_.ptr<cv::Vec2d>();
    cv::Vec2d *depth_lookup_rectified_y_ptr = depth_lookup_rectified_y_.ptr<cv::Vec2d>();
    const double fx = 1.0 / ir_optimal_camera_matrix_.at<double>(0);
    const double fy = 1.0 / ir_optimal_camera_matrix_.at<double>(4);
    const double cx = ir_optimal_camera_matrix_.at<double>(2);
    const double cy = ir_optimal_camera_matrix_.at<double>(5);

    for(int i = 0 ; i < depth_lookup_rectified_x_.rows ; ++i) {
      for(int j = 0 ; j < depth_lookup_rectified_x_.cols ; ++j) {
        *depth_lookup_rectified_x_ptr = (i - cx) * fx;
        *depth_lookup_rectified_y_ptr = (j - cy) * fy;
